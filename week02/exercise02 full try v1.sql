...
SELECT 
    ...
    -- ISSUE: Records have blank/empty category values, making it impossible
    ...
    --        in 167 out of 200 records. During CSV import, Snowflake stored 
    --        2-digit years as literal years (e.g., '44' became year 0044, not 
    --        1944). This correction applies the standard pivot: years 1-24 
    --        become 2001-2024, years 25-99 become 1925-1999.
    -- IMPACT: Age restrictions for marketing (e.g., can't contact minors), 
    --         demographic analysis, and donor profiling become unreliable
    -- RULE: Calculate age from DOB with century correction and flag if 
    --       difference from AGE field exceeds 5 years
    -- NOTE: This check handles the date import issue where 2-digit years were
    --       stored as literal values (0001-0099) instead of with proper centuries
    -- ========================================================================
    CASE 
        WHEN ABS(
            age - (
                2024 - (
                    CASE 
                        WHEN YEAR(date_of_birth) <= 24 THEN YEAR(date_of_birth) + 2000
                        WHEN YEAR(date_of_birth) < 100 THEN YEAR(date_of_birth) + 1900
                        ELSE YEAR(date_of_birth)
                    END
                )
            )
        ) > 5 
    ...
    (
        ...
        CASE WHEN ABS(age - (2024 - (CASE WHEN YEAR(date_of_birth) <= 24 THEN YEAR(date_of_birth) + 2000 WHEN YEAR(date_of_birth) < 100 THEN YEAR(date_of_birth) + 1900 ELSE YEAR(date_of_birth) END))) > 5 THEN 1 ELSE 0 END +
...
1. AGE/DATE OF BIRTH MISMATCH (167 records affected - 83.5%)
   - This is the most severe issue, affecting the vast majority of records
   - The AGE field doesn't match the calculated age from DATE_OF_BIRTH
   - Many ages are off by 20-30 years or more from what DOB indicates
   - Root cause: During CSV import, Snowflake stored 2-digit years as literal 
     years (e.g., '44' became 0044 instead of 1944), requiring century correction
   - The AGE field appears to have been entered at one point in time and never
     updated, while DOB remained static
   - Recommendation: Drop the AGE field entirely and always calculate age
     from DATE_OF_BIRTH at query time with proper century handling. 
     Alternatively, implement a database trigger to auto-update AGE whenever 
     DOB changes.

2. NAME FORMAT INCONSISTENCY (90 records affected - 45%)
...
3. PHONE NUMBER FORMAT INCONSISTENCY (87 records affected - 43.5%)
   - Multiple competing formats make phone number validation and automated
     dialing impossible without preprocessing
   - Formats include: (123) 456-7890, 123-456-7890, 123.456.7890, 
     1-234-567-8901x123, +1-234-567-8901
...
4. PLACEHOLDER CATEGORIES (32 records affected - 16%)
   - Records use "Unknown" or "N/A" as category values
   - While technically not null, these are placeholder values that 
     provide no meaningful classification
   - Recommendation: Review these records and attempt to classify based on
     organization name or other contextual clues

5. MISSING CATEGORIES (31 records affected - 15.5%)
   - 31 records have completely blank category values
   - Combined with placeholder categories, this means 31.5% of records 
     (63 total) lack proper classification
...
7. OUTLIER DONATION AMOUNTS (8 records affected - 4%)
...
- COMPLIANCE: Age verification is unreliable with 83.5% of records having 
  incorrect age data, creating potential legal issues around contacting minors 
  or meeting regulatory requirements.

- DONOR COMMUNICATION: Name formatting and phone number issues (affecting 45% 
  and 43.5% of records respectively) make it difficult to reach donors reliably. 
  Mail merge operations may fail or produce incorrectly formatted letters. 
  Automated calling systems will fail on nearly half the database.

- ANALYTICS: Demographic analysis and donor segmentation are unreliable when
  83.5% of records have bad age data and 31.5% of records lack meaningful 
  categories. Any reports based on age or detailed segmentation cannot be trusted.
...
operational compliance and demographic analysis. The second priority should be 
standardizing phone numbers to enable reliable contact. The outlier amounts 
should be reviewed immediately to ensure financial reporting accuracy.
